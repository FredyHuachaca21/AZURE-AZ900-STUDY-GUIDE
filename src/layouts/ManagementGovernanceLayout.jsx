// Ruta: /src/layouts/ManagementGovernanceLayout.jsx
...
const navLinks = [
  { name: 'Administración de costos', path: 'cost-management' },
  { name: 'Gobernanza y cumplimiento', path: 'governance' },
  { name: 'Administración e implementación', path: 'resource-management' },
  { name: 'Herramientas de supervisión', path: 'monitoring' }
];

export default function ManagementGovernanceLayout() {
  ...
  const getCurrentPageTitle = () => {
    ...
    return currentLink ? currentLink.name : 'Administración y gobernanza de Azure';
  };
    ...
    <div className="flex flex-col md:flex-row gap-8">
      ...
      <aside className="md:w-1/4 bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 transition-colors duration-200">
        <div className="flex items-center space-x-2 mb-6 text-azure-700 dark:text-azure-400">
          ...
          <h2 className="text-lg font-semibold">Administración y Gobernanza</h2>
        </div>
        ...
        <nav className="space-y-1">
          {navLinks.map((link) => (
            <NavLink
              key={link.path}
              to={`/management-governance/${link.path}`}
              className={({ isActive }) =>
      ...
      <div className="md:w-3/4 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 transition-colors duration-200">
        <div className="mb-6 pb-4 border-b border-gray-200 dark:border-gray-700">
          <div className="text-sm breadcrumbs text-gray-500 dark:text-gray-400 mb-2">
            <span>AZ-900</span> &gt; <span>Administración y gobernanza</span> &gt; <span>{getCurrentPageTitle()}</span>
          </div>
